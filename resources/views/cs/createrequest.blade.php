...
<div class="col-xl">
    <div class="card mb-4">
      <div class="card-body">
        <form method="POST" enctype="multipart/form-data" action="{{ isset($form)?route('create-requestlist'):route('update-requestlist', $data->id) }}">
        @csrf
          <div class="mb-3">
            ...
            <input class="form-control" type="file" id="photo_payment" name="photo_payment" aria-describedby="buttonView">
              @if ($form == 'edit')
                @if ($data->photo_payment != null)
                  <button class="btn btn-outline-primary" type="button" id="buttonView"><a href="{{ asset('/storage/payment/'.$data->photo_payment) }}" data-lightbox="photos" target="_blank">Lihat Foto</a></button>
                @endif
              @endif
            </div>
          </div>
            ...
            <div class="input-group input-group-merge">
              ...
              @if ($form == 'edit')
                @if ($data->photo_taker != null)
                  <button class="btn btn-outline-primary" type="button" id="buttonView">Lihat Foto</button>
                @endif
              @endif
            </div>
          </div>
